...
function init() {
    ...
    */
    ...
    span = document.querySelector('.score');
...
function startGame() {
    ...
    snake_problem = setInterval(createProblem, 5000);
}
function respawn() {
    // Змейка - массив td
...
function move() {
    ...
    if (direction == 'x-') {
        ...
        if (coord_x == 0) {
            new_unit = document.querySelector('.cell-' + (coord_y) + '-' + 19);
        }
    } else if (direction == 'x+') {
        new_unit = document.querySelector('.cell-' + (coord_y) + '-' + (coord_x + 1));
        if (coord_x == 19) {
        ...
        if (coord_y == 0) {
            new_unit = document.querySelector('.cell-' + 19 + '-' + (coord_x));
        }
    } else if (direction == 'y-') {
        new_unit = document.querySelector('.cell-' + (coord_y + 1) + '-' + (coord_x));
        if (coord_y == 19) {
    ...
    // 2) Змейка не попала на бомбочку
    //console.log(new_unit);

    if (!isSnakeUnit(new_unit) && !isProblemUnit(new_unit)) {
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        createFood();
        score++;
        span.innerHTML = 'Текущий счет: ' + score;
    }
    return check;
}
...
function createProblem() {
    ...
    while (!problemCreated) { //пока бомбочку не создали
        // рандом
